...
public final class DBOperations {
    ...
    //Operations of  DeptManager
    public Cursor getDeptManager(){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s",
                OrderDB.Tables.DEPT_MANAGER);
        return  database.rawQuery(sql, null);
    }

    public Cursor getDeptManagerById(String id){
        SQLiteDatabase database = db.getReadableDatabase();
        String sql = String.format("SELECT * FROM %s" +
                        " WHERE %s=?",
                OrderDB.Tables.DEPT_MANAGER,
                OrderContract.DeptManagers.ID);
        String[] whereArgs = {id};
        return database.rawQuery(sql, whereArgs);
    }


    public String insertDeptManager(DeptManager deptManager){
        SQLiteDatabase database = db.getWritableDatabase();
        String idDeptManager = OrderContract.DeptManagers.generateIdDeptManager();
        ContentValues values = new ContentValues();
        values.put(OrderContract.DeptManagers.ID, idDeptManager);
        values.put(OrderContract.DeptManagers.NAMEDEPT, deptManager.getNameDept());
        values.put(OrderContract.DeptManagers.FROMDATE, deptManager.getFromDate());
        values.put(OrderContract.DeptManagers.TODATE, deptManager.getToDate());
        database.insertOrThrow(OrderDB.Tables.DEPT_MANAGER, null, values);
        return idDeptManager;
    }

    public boolean updateDeptManager(DeptManager deptManager){
        SQLiteDatabase database = db.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(OrderContract.DeptManagers.NAMEDEPT, deptManager.getNameDept());
        values.put(OrderContract.DeptManagers.FROMDATE, deptManager.getFromDate());
        values.put(OrderContract.DeptManagers.TODATE, deptManager.getToDate());
        String whereClause = String.format("%s=?", OrderContract.DeptManagers.ID);
        String[] whereArgs = {deptManager.getIdDept()};
        int result = database.update(OrderDB.Tables.DEPT_MANAGER,
                values, whereClause, whereArgs);
        return result>0;
    }

    public boolean deleteDept(String idDeptManager){
        SQLiteDatabase database = db.getWritableDatabase();
        String whereClause = OrderContract.DeptManagers.ID + "=?";
        String[] whereArgs = {idDeptManager};
        int result =  database.delete(OrderDB.Tables.DEPT_MANAGER,
                whereClause, whereArgs);
        return result > 0;
    }
